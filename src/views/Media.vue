  ...
  <div class="font-sans text-black min-h-screen bg-primary-600 p-5">
    <div class="wrapper border border-primary-100 bg-white">
      <div class="title flex justify-between py-8 m-4">
        ...
        <button class="button button-upload py-2 px-4 rounded-md bg-green-200 text-white"><i class="fa-solid fa-file-arrow-up mr-2"></i>Upload Media</button>
      </div>
      <div class="border border-l-0 border-r-0 border-primary-100 px-4 flex justify-between">
        <div class="">
          <BackgroundTab
            :data="allTypes"
            v-model="currentType"
        ...
        <div class="flex justify-between my-1">
          <div class="border rounded overflow-hidden flex mr-4 text-primary-400">
            <select name="sort" id="sort" class="bg-white">
              <option v-for="(item, idx) in sortTypes" :key="idx" :value="item.id">{{ item.title }}</option>
            </select>
            <button
